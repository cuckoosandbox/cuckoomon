...
int main(void)
{
	...
	{
		fprintf( stderr, "Error loading cuckoomon.dll" );
		return 1;
	}
    ...
    for (uint32_t tid = 2000; ; tid += 4) {
        ...
        if(thread_handle != NULL) {
            printf("tid %d .. :)\n", tid);
            if ( QueueUserAPC((PAPCFUNC) sleep, thread_handle, 1337) != 0 )
				injected = TRUE;
	...
	if ( injected == FALSE )
		fprintf( stderr, "Error QueueUserAPC" );

	return (injected == TRUE) ? 0 : 1;
}
